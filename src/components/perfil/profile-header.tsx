...
export function ProfileHeader({ perfil, editavel = true }: ProfileHeaderProps) {
  ...
  const bannersEspeciais = [
    ...(perfil.isPro ? [{ id: "pro-banner", name: "Banner PRO", gradient: BANNER_PRO }] : []),
    ...(perfil.isDeveloper ? [{ id: "developer-banner", name: "Banner Desenvolvedor", gradient: BANNER_DEV }] : []),
    ...(perfil.isEarlyTester ? [{ id: "early-tester-banner", name: "Banner Pioneiro", gradient: BANNER_TESTER }] : []),
    ...(conquistouRichard ? [{ id: "richard-tribute-banner", name: "Banner Richard", gradient: BANNER_RICHARD }] : []),
  ];
        ...
        {editavel && <div className="absolute right-3.5 top-3.5" ref={seletorRef}>
          {seletorAberto && (
            ...
            >
              ...
              {[...bannersEspeciais, ...bannersComprados].length > 0 && <div className="mt-3 border-t border-border pt-3"><p className="mb-2 text-[0.65rem] font-black uppercase tracking-wider text-muted-foreground">Banners disponíveis</p><div className="grid gap-2">{[...bannersEspeciais, ...bannersComprados].map((banner) => <button key={banner.id} type="button" onClick={() => escolherBannerComprado(banner)} className="flex items-center gap-2 rounded-xl p-1.5 text-left text-xs font-bold hover:bg-muted"><span className="h-8 w-12 rounded-lg" style={{ background: banner.gradient }} /><span className="truncate">{banner.name}</span></button>)}</div></div>}
            </div>
          )}
        </div>}
      </div>
