...
function MetasContent() {
  const { metas, especiais, conectada, resgatar } = useMetasDiarias();
  ...
  return (
    <div className="pt-3">
      ...
      {aba === "especiais" ? <section className="mt-5"><h2 className="text-lg font-black">Missões especiais</h2><p className="mt-1 text-sm text-muted-foreground">Desafios permanentes que marcam sua jornada.</p><div className="mt-3 rounded-[20px] border border-border bg-card px-4">{especiais.map(m => { const Icon=m.icon === "rank" ? Trophy : m.icon === "unit" ? Flag : Award; const pct=Math.min(100,Math.round(m.current/m.target*100)); return <div key={m.id} className={`flex gap-4 border-b border-border py-5 last:border-0 ${m.unavailable ? "opacity-55" : ""}`}><Icon className="mt-1 size-8 shrink-0 text-violet-500"/><div className="min-w-0 flex-1"><p className="font-black">{m.title}</p><p className="mt-1 text-sm text-muted-foreground">{m.description}</p><div className="relative mt-3 h-5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-violet-500" style={{width:`${pct}%`}}/><span className="absolute inset-0 flex items-center justify-center text-xs font-black">{m.current}/{m.target}</span></div></div></div>})}</div></section> : (["DAILY", "WEEKLY", "MONTHLY"] as const).map(periodo => {
