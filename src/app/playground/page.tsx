...
function Playground() {
  ...
  return <div className="flex h-full min-h-0 flex-col py-2 lg:py-3">
    ...
    <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden border-x border-b border-border bg-background lg:grid-cols-[190px_minmax(0,1fr)_minmax(330px,.85fr)]">
      <aside className={`${mobilePanel === "files" ? "flex" : "hidden"} min-h-0 flex-col border-r border-border bg-card lg:flex`}><div className="flex items-center gap-2 border-b border-border px-3 py-3 text-xs font-black text-foreground"><FolderOpen className="size-4 text-primary" /> Arquivos</div><div className="p-2">{files.map(({ id, label, Icon }) => <button key={id} onClick={() => selectFile(id)} className={`flex w-full items-center gap-2 rounded-md px-2.5 py-3 text-left text-xs font-bold transition-colors ${tab === id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}><Icon className="size-3.5" />{label}</button>)}</div><div className="mt-auto border-t border-border p-3 text-[10px] leading-relaxed text-muted-foreground"><ShieldCheck className="mb-1 size-4 text-primary" />Execução limitada, sem acesso à rede ou aos dados do ZulCode.</div></aside>
      <section className={`${mobilePanel === "editor" ? "grid" : "hidden"} min-h-0 grid-rows-[42px_minmax(0,1fr)_minmax(130px,30%)] border-b border-border lg:grid lg:grid-rows-[42px_minmax(0,1fr)_190px] lg:border-b-0 lg:border-r`}><div className="zc-scroll-hidden flex items-end gap-1 overflow-x-auto border-b border-border bg-card px-2">{files.map(({ id, label, Icon }) => <button key={id} onClick={() => setTab(id)} className={`flex h-10 shrink-0 items-center gap-2 border-t-2 px-3 text-xs font-bold transition-colors ${tab === id ? "border-primary bg-background text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`}><Icon className="size-3.5" />{label}</button>)}</div><textarea value={value} onChange={e => change(e.target.value)} spellCheck={false} className="zc-scroll-hidden min-h-0 w-full resize-none bg-background p-3 font-mono text-[13px] leading-6 text-foreground outline-none sm:p-4" /><div className="flex min-h-0 flex-col border-t border-border bg-muted/35"><div className="flex items-center gap-2 border-b border-border px-3 py-2 text-xs font-black text-foreground"><Terminal className="size-3.5 text-primary" /> Terminal</div><div className="zc-scroll-hidden min-h-0 flex-1 overflow-y-auto p-3 font-mono text-xs leading-5">{logs.length ? logs.map(log => <p key={log.id} className={log.tone === "error" ? "text-destructive" : log.tone === "success" ? "text-emerald-500" : "text-foreground"}><span className="mr-2 text-muted-foreground">›</span>{log.text}</p>) : <p className="text-muted-foreground">$ pronto para executar</p>}</div></div></section>
      <section className={`${mobilePanel === "preview" ? "flex" : "hidden"} min-h-0 min-w-0 flex-col bg-card lg:flex`}><div className="flex items-center gap-2 border-b border-border bg-muted/40 px-3 py-2 text-xs font-bold text-foreground"><ChevronRight className="size-3.5" /><span className="rounded bg-background px-2 py-1">Preview:/</span><span className="ml-auto flex items-center gap-1 text-emerald-500"><Monitor className="size-3.5" /> Web output</span></div>{web ? preview ? <iframe ref={iframe} title="Preview protegido" sandbox="allow-scripts" referrerPolicy="no-referrer" srcDoc={preview} className="min-h-0 flex-1" /> : <div className="grid flex-1 place-items-center text-center text-sm text-muted-foreground"><div><Eye className="mx-auto mb-2 size-7" />Clique em Rodar para ver o preview.</div></div> : <div className="grid flex-1 place-items-center p-6 text-center text-sm text-muted-foreground"><div><Terminal className="mx-auto mb-2 size-7" />{selectedLanguage} usa saída pelo terminal.</div></div>}</section>
    </div>
  </div>;
}
