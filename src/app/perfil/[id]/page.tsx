  ...
  useEffect(() => {
    ...
    fetchComTimeout(`${API_BASE_URL}/user/public/${id}`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then((data) => {
        ...
        setPerfil({ id: data.id, publicCode: data.publicCode, role: "USER", nome: data.name, email: "", iniciais: iniciais(data.name), avatarId: data.avatarId, bannerColor: data.bannerColor, xp: data.xp, nivel: data.nivel, nivelLabel: data.nivelLabel, xpNivelAtual: progresso.xpNivelAtual, xpNecessarioNivel: progresso.xpNecessarioNivel, xpProximoNivel: data.xpProximoNivel, streakAtual: data.currentStreak, streakRecorde: data.longestStreak, vidas: null, moedas: null, xpHoje: null, licoesHoje: null });
      })
      .catch(() => setErro(true));
  }, [id]);

  return <AppShell contentClassName="max-w-6xl"><div className="pt-3">{!perfil && !erro && <div className="flex min-h-[55dvh] items-center justify-center"><span className="size-8 animate-spin rounded-full border-2 border-muted border-t-primary" /></div>}{erro && <div className="flex min-h-[55dvh] flex-col items-center justify-center gap-4"><p className="text-muted-foreground">Não foi possível carregar este perfil.</p><Button onClick={() => location.reload()}>Tentar novamente</Button></div>}{perfil && <div className="flex flex-col gap-7"><ProfileHeader perfil={perfil} editavel={false} /><StatsGrid perfil={perfil} /><div className="rounded-[20px] border border-border bg-card p-5 text-sm text-muted-foreground">{perfil.nome} concluiu desafios e está evoluindo no ZulCode.</div></div>}</div></AppShell>;
}
